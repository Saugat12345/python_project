...
class MainPage(Frame):
    ...
    def __init__(self, master):
        ...
        self.title_label = Label(self.master, font=("Helvetica", 16),
                                 text="Welcome to ZZ Theatre!! Please select whether"
...
class CustMain(Customer):
    ...
    def __init__(self, master):
        ...
        self.defdate1.set("Monday 1/01/20")  # default value
        self.datelist1 = OptionMenu(master, self.defdate1, "Monday 1/01/20", "Tuesday 2/01/20", "Wednesday 3/01/20",
                                    "Thursday 4/01/20", "Friday 5/01/20", "Saturday 6/01/20", "Sunday7/01/20")
        self.datelist1.pack()
...
class SearchResults(CustMain):
    def __init__(self, master, output, search_date):
            ...
            self.title = Label(self.master, text='Title', font=("Helvetica", 16), width=15).grid(row=0, column=3,
                                                                                                 pady='1')
            self.description = Label(self.master, text='Description', font=("Helvetica", 16), width=30).grid(row=0,
                                                                                                             column=4,
                                                                                                             pady='1')
            self.booked = Label(self.master, text='Booked', font=("Helvetica", 16), width=8).grid(row=0, column=5,
                                                                                                  pady='1')
    ...
    def boo(self, gone, datetime7):
        ...
        if msg:
            ...
            if alr:
                ...
                if td_year > 2020 or \
                        td_year == 2020 and td_month > 1 or \
                        td_year == 2020 and td_month == 1 and td_day > new_date or \
                        td_year == 2020 and td_month == 1 and td_day == new_date and td_hour >= new_time:
...
class BookHist(Booked):
    def __init__(self, master):
                ...
                self.seatno = Label(self.master, text='Seat Number', font=("Helvetica", 16), width='15').grid(row=0,
                                                                                                              column=3)
                self.remo = Label(self.master, text='Remove Booking', font=("Helvetica", 16), width='15').grid(row=0,
                                                                                                               column=4)

                c3.execute('''SELECT date, time, seatno FROM bookings WHERE
                            username = ? ORDER BY date='Monday 1/01/20' DESC,
                                                            date='Tuesday 2/01/20' DESC,
                                                            date='Wednesday 3/01/20' DESC,
                                                            date='Thursday 4/01/20' DESC,
                                                            date='Friday 5/01/20' DESC,
                                                            date='Saturday 6/01/20' DESC,
                                                            date='Sunday 7/01/20' DESC,
                                                            time='1pm' DESC,
                                                            time='2pm' DESC,
                                                            time='3pm' DESC,
                                                            time='4pm' DESC,
    ...
    def remove(self, date1, time):
                ...
                if msg:
                    ...
                    if td_year > 2020 or \
                        ...
                        tkinter.messagebox.showinfo("---- REMOVED ----", "Film removed from booking history.",
                                                    icon="info")
                        self.newWindow = Toplevel(self.master)
                        self.newWindow.geometry('1096x720')
                        self.app = BookHist(self.newWindow)
...
class Showings(EmployeeMain):
    ...
    January 2020, and times range from 1-11pm (with the last showing at 10pm). The shows are for exactly
    one hour'''
    ...
    def __init__(self, master):
        ...
        self.seeseats = Label(self.master, text='Seating', font=("Helvetica", 16), width=8).grid(row=0, column=7,
                                                                                                 pady='1')
        c2.execute('''SELECT * FROM movies ORDER BY date='Monday 1/01/20' DESC,
                                                    date='Tuesday 2/01/20' DESC,
                                                    date='Wednesday 3/01/20' DESC,
                                                    date='Thursday 4/01/20' DESC,
                                                    date='Friday 5/01/20' DESC,
...
class SeeSeatsEmp(Showings):
    def __init__(self, master, booked):
            ...
            self.title = Label(self.master, text='Seat layout:    green = available,'
                                                 ' red = unavailable', font=("Helvetica", 16)).grid(row=0,
                                                                                                    columnspan=94)
            tens = self.booked // 10
            rem = self.booked % 10
            for i in range(tens):
    ...
    def Customer_Database_Initialiser(self):
    ...
    def Films_Database_Initialiser(self):
    ...
    def Bookings_Database_Initialiser(self):
    ...
    def main(self):
        window = Tk()
        window.title("ZZ Theatre !!")
        window.geometry('1096x720')
        self.Customer_Database_Initialiser()
        self.Films_Database_Initialiser()
        self.Bookings_Database_Initialiser()
        label = Label(window)
